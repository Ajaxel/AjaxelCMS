...
* 
* @file       tpls/admin/tree_window.php
...
?><?php
$title = lang('$'.($this->id ? 'Edit':'Add new').' tree element:').($this->id?' (ID: '.$this->id.')':'').' ';
		...
		$('#a-w-position_<?php echo $this->name_id?>').change(function(){
			...
			if(o.value=='#new'){
				S.A.F.addOpt(this, ['<?php echo lang('_$New position name')?>','<?php echo lang('_$Add')?>'], false, function(v){
					...
					S.A.F.fillOptions('a-parent-tree_<?php echo $this->name_id?>','?<?php echo URL_KEY_ADMIN?>=<?php echo $this->name?>',{
						get:'parent_trees',
						position:v
					}, true)
				});
				...
				$('#a-parent-tree_<?php echo $this->name_id?>').find('option').remove().end();
			}
		});
...
<form method="post" class="window_form" id="a-form_<?php echo $this->name_id?>">
	<table class="a-form" cellspacing="0">
		...
		<tr>
			<td class="a-l"><?php echo lang('$Position:')?></td><td class="a-r"><select name="data[position]" id="a-w-position_<?php echo $this->name_id?>" style="width:95%" class="a-select"><option value=""><?php echo lang('$-- select --')?></option><?php echo Html::buildOptions($this->post('position', false), $this->Tree->getPositions(), true)?><option value="#new"> -- create new --</select></div></td>
			<td class="a-l2"><?php echo lang('$Parent tree:')?></td><td class="a-r"><select name="data[parentid]" id="a-parent-tree_<?php echo $this->name_id?>" style="width:95%" class="a-select"><option value="0"><?php echo lang('$-- top level --')?><?php 
			echo $this->Tree->get()->selected($this->post('parentid', false))->not($this->id)->toOptions();
			?></select></td>
		</tr>
		<tr>
			<td class="a-l"><?php echo lang('$Keywords:')?></td><td class="a-r" colspan="3"><input type="text" class="a-input" name="data[keywords]" value="<?php echo $this->post('keywords')?>" /></td>
		</tr>
		<tr>
			<td class="a-l2"><?php echo lang('$Description:')?></td><td class="a-r" colspan="3"><textarea class="a-textarea" style="width:95%;height:30px" name="data[descr]"><?php echo $this->post('descr')?></textarea></td>
		</tr>

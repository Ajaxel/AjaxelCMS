...
* 
* @file       tpls/admin/settings_templates_create_window.php
...
$this->title = ($this->post['name']?lang('$Edit %1 template',$this->post['name']):lang('$Create new template'));
$this->height = 320;
$this->width = 400;
?>
...
var window_<?php echo $this->name_id?> = {
	...
	,load:function() {
		...
		S.A.FU.init(65);
		$('#a-w-<?php echo $this->name_id?>_name').blur(function(){
			var v=$('#a-w-<?php echo $this->name_id?>_name').val();
			if (!$('#a-w-<?php echo $this->name_id?>_prefix').val()) {
				if (v) $('#a-w-<?php echo $this->name_id?>_prefix').val(v+'_');
			}
...
?><form method="post" class="window_form" id="a-form_<?php echo $this->name_id?>">
	<table class="a-form" cellspacing="0">
		<tr>
			<td class="a-l" width="25%" nowrap><?php echo lang('$Folder name:')?></td><td class="a-r" width="85%"><input class="a-input" type="text" name="data[name]" value="<?php echo $this->post('name')?>" id="a-w-<?php echo $this->name_id?>_name" style="width:60%" /></td>
		</tr>
		<tr>
			<td class="a-l"><?php echo lang('$Data prefix:')?></td><td class="a-r"><input class="a-input" type="text" name="data[prefix]" value="<?php echo $this->post('prefix')?>" id="a-w-<?php echo $this->name_id?>_prefix" style="width:60%" /></td>
		</tr>
		<tr>
		...
		<tr>
			<td class="a-l"><?php echo lang('$Duplicate%1 template files%1 from:','<br />')?></td><td class="a-r"><select style="width:50%" size="5" class="a-select" type="text" name="data[files_from]"><?php echo Html::buildOptions($this->post('files_from', false), array_label($this->templates))?></select></td>
		</tr>
		<tr>
			<td class="a-l"><?php echo lang('$Duplicate%1 data%1 from:','<br />')?></td><td class="a-r"><select style="width:50%" size="5" type="text" class="a-select" name="data[data_from]"><option value="" selected="selected"><?php echo lang('$use empty')?></option><?php echo Html::buildOptions($this->post('data_from', false), array_label($this->templates))?></select></td>
		</tr>
		<?php /*
		<tr>
	...
	<div class="a-window-bottom ui-dialog-buttonpane">
		<table width="100%"><tr>
		<td class="a-td2">
			<?php $this->inc('button',array('click'=>'S.A.W.save(\'?'.URL_KEY_ADMIN.'=settings&'.self::KEY_TAB.'='.$this->tab.'&'.self::KEY_ACTION.'=create\', this.form, this);','img'=>'oxygen/16x16/actions/document-save.png','text'=>lang('$Create'))); ?>
		</td>
		</tr></table>
	</div>

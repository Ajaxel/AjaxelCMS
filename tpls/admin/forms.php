...
$().ready(function() {
	...
	if (data.length) {
		...
		for(i=0;i<data.length;i++) {
			...
			html += '<td class="a-l" width="30%">'+a.name+'</td>';
			...
			html += '<a href="javascript:;" onclick="S.A.L.del({id:'+a.id+', active:'+(a.active==1)+', title: \''+S.A.P.js(a.login)+'\'}, this)"><img src="<?php echo FTP_EXT?>tpls/img/oxygen/16x16/actions/edit-delete.png" /></a>';
			html += '</td>';
		...
		var html = '<div class="a-not_found"><?php echo lang('$No results were found','forms')?></div>';
	}
	S.A.L.ready(html);
});
</script>

...
* 
* @file       tpls/admin/email_log_tab.php
...
$().ready(function() {
	...
	if (j) {
		...
		html += '<tr><th width="50%"><?php echo lang('$Title')?></th><th width="45%"><?php echo lang('$Passes')?></th><th width="5%">&nbsp;</th></tr>';
		for (i in data) {
			a = data[i];
			html += '<tr class="'+(i%2?'':'a-odd')+'">';
			html += '<td class="a-l"><a href="javascript:;" onclick="S.A.M.edit('+a.id+', this)">'+a.title+'</a></td>';
			html += '<td class="a-l">'+a.passes+'</td>';
			...
			html += '<a href="javascript:;" onclick="S.A.L.act(\'?<?php echo URL_KEY_ADMIN?>=poll\', {id:'+a.id+', active:'+(a.active==1)+', title: \''+S.A.P.js(a.title)+'\'}, this)"><img src="<?php echo FTP_EXT?>tpls/img/oxygen/16x16/actions/flag-'+(a.active==1?'green':'red')+'.png" alt="<?php echo FTP_EXT?>tpls/img/oxygen/16x16/actions/flag-'+(a.active==1?'red':'green')+'.png" /></a>';
			html += '<a href="javascript:;" onclick="S.A.L.del({id:'+a.id+', active:'+(a.active==1)+', title: \''+a.title+'\'}, this)"><img src="<?php echo FTP_EXT?>tpls/img/oxygen/16x16/actions/edit-delete.png" /></a>';
			html += '</td>';			
			html += '</tr>';
		}
		html += '</table>';

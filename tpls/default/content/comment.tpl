{strip}
<li class="comment {if $row.userid==1}admincomment{else}regularcomment{/if}" id="{$row.id}" onmouseover="$('#act_{$row.id}').show()" onmouseout="$('#act_{$row.id}').hide()">
	<div class="author">
		<a name="{$row.id}"></a>
	...
	<div class="info">
		<div class="date">{'d/m/y H:i'|date:$row.added} | <a title="permalink: test" href="#{$row.id}">#</a>&nbsp;{$row.subject} {if $row.url} | <a href="{$row.url}" target="_blank">www</a>{/if}</div>

	...
	private function run() {
		...
		*/
		...
		elseif (get('category') && ($cat = DB::row('SELECT * FROM '.$this->prefix.'forum_categories WHERE id='.(int)get('category')))) {
			...
			$data['type'] = 'threads';
			if (!$reply) {
				$qry = DB::qry('SELECT SQL_CALC_FOUND_ROWS id, title, descr, views, posts, added, userid, user FROM '.$this->prefix.'forum_threads WHERE catid='.$cat['id'].' ORDER BY id DESC',$this->My->offset,$this->My->limit);
				$this->My->total = DB::rows();
			} else {

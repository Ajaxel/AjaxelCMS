...
(function ($) {
	...
	$.fn.chili = function (options) {
			...
			$(el).parents().filter("pre").bind("mousedown", function () {
				...
				if (element && (element == this)) {
					...
					if ($.browser.msie) {
						...
						selected = selected.replace(/\r/g, '').replace(/^# ?/g, '').replace(/\n# ?/g, '\n');
						var container_tag = '<pre style="STYLE">';
					}
					var $container = $(container_tag.replace(/\bSTYLE\b/, ChiliBook.selectionStyle)).appendTo('body').text(selected).attr('id', 'chili_selection').click(function () {
						$(this).remove();
					});
		...
		function addLineNumbers(el) {
			...
			.replace(/(.*?)(<span .*?>)(.*?)(?:<\/span>(?:&nbsp;|\xA0)<br>|<\/span>)/ig,
			function (all, before, open, content) {
				if (/<br>/i.test(content)) {
					...
					+ (lastPiece + content).replace(/((.*?)(?:&nbsp;|\xA0)<br>)|(.*)/ig, function (tmp, not_last_line, not_last, last) {

	...
	<ul class="pagenavi{if $top} top{/if}">
		<li class="newer">
			{if $pager.page}
				...
				{else}
					<a href="{$pager.url}{if $pager.page>1}&{$pager.page_key}={$pager.page-1}{/if}" class="ajax_link">{'prev'|lang}</a>
				{/if}
			{/if}
		</li>
		...
		<li class="page{if $s} selected{/if}">
			...
			{else}
				<a href="{$pager.url}{if $p-1>0}&{$pager.page_key}={$p-1}{/if}" class="ajax_link">{$p}</a>
			{/if}
		</li>
		{/foreach}
		{if $pager.next_page}
			<li class="older">
				...
				{else}
					<a href="{$pager.url}&{$pager.page_key}={$pager.next_page}" class="ajax_link">{'next'|lang}</a>
				{/if}
			</li>
		{/if}
	</ul>

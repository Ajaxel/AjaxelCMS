...
switch($texte)
	...
	break;
	case 'right':
	$font = FTP_DIR_FONTS.$fontesmath[$texte].'.ttf';
	$t=16;
	...
	case '_dintegrale':
	case '_tintegrale':
	if(isset($fontesmath[$texte])) $font = FTP_DIR_FONTS.$fontesmath[$texte].'.ttf';
	elseif (est_nombre($texte)) $font = FTP_DIR_FONTS.'cmr10.ttf';
	else $font = FTP_DIR_FONTS.'cmmi10.ttf';
	$t=6;
...
$texte=stripslashes($texte);
if(isset($fontesmath[$texte])) $font = FTP_DIR_FONTS.$fontesmath[$texte].'.ttf';
elseif (ereg("[a-zA-Z]", $texte)) $font = FTP_DIR_FONTS.'cmmi10.ttf';
else $font = FTP_DIR_FONTS.'cmr10.ttf';
if(isset($symboles[$texte])) $texte = $symboles[$texte];
$htexte = 'dg'.$texte;
$hdim = ImageTTFBBox($taille, 0, $font, $htexte);
$wdim = ImageTTFBBox($taille, 0, $font, $texte); 

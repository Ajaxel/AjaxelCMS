...
class MacBinary {
	...
	function loadHeader() {
		...
		if( strlen( $head ) < 128 ) {
			$this->debug( "$fname: couldn't read full MacBinary header");
			return false;
		}

		if( $head{0} != "\x00" || $head{74} != "\x00" ) {
			$this->debug( "$fname: header bytes 0 and 74 not null");
			return false;
		}
		...
		if( $storedCRC == $calculatedCRC ) {
			...
			if( $storedCRC == 0 && $head{82} == "\x00" &&
				...
				$this->debug( "$fname: no CRC, looks like MacBinary I");
				$this->version = 1;
			} elseif( $signature == 'mBIN' && $storedCRC == 0x185 ) {
				$this->debug( "$fname: CRC doesn't match ($crc), looks like Mac IE 5.0");
				$this->version = 3;
			} else {
				$this->debug( "$fname: CRC doesn't match ($crc) and not MacBinary I");
				return false;
			}
		}
		...
		if( $nameLength < 1 || $nameLength > 63 ) {
			$this->debug( "$fname: invalid filename size $nameLength");
			return false;
		}
		...
		if( $this->dataLength < 0 || $this->dataLength > $maxForkLength ) {
			$this->debug( "$fname: invalid data fork length $this->dataLength");
			return false;
		}

		if( $this->resourceLength < 0 || $this->resourceLength > $maxForkLength ) {
			$this->debug( "$fname: invalid resource fork size $this->resourceLength");
			return false;
		}

		$this->debug( "$fname: appears to be MacBinary $this->version, data length $this->dataLength");
		$this->valid = true;
		return true;
	}

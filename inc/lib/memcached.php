...
class memcached {
	...
	*/
	function delete($key, $time = 0) {
		...
		if ($_GET['dump']=='MEM') $this->_debugprint(sprintf("MemCache: delete %s (%s)\n", $key, $res));
		
		if ($res == "DELETED") return true;
		return false;
	}
	function disconnect_all() {
		foreach ($this->_cache_sock as $sock) fclose($sock);
		$this->_cache_sock = array();
	}
	...
	*/
	...
	function get ($key)	{
		...
		if ($_GET['dump']=='MEM') $this->_debugprint("get($key)\n");
		if (!$this->_active) {
			return false;
		}
		...
		if ($_GET['dump']=='MEM') {
			foreach ($val as $k => $v) $this->_debugprint(sprintf("MemCache: sock %s got %s\n", serialize($sock), $k));
		}
		return @$val[$key];
	}
	...
	* @access  public
	*/
	function set ($key, $value, $exp=3600)
	{
	...
	function _load_items ($sock, &$ret) {
	  while (1) {
		 ...
		 if ($decl == "END\r\n") {
			...
			if ($offset != $len+2) {
			   // Something is borked!
			   if ($_GET['dump']=='MEM') $this->_debugprint(sprintf("Something is borked!  key %s expecting %d got %d length\n", $rkey, $len+2, $offset));
			   unset($ret[$rkey]);
			   $this->_close_sock($sock);
			   return false;
			}
	
			if ($this->_have_zlib && $flags & memcached::COMPRESSED) $ret[$rkey] = gzuncompress($ret[$rkey]);
			$ret[$rkey] = rtrim($ret[$rkey]);
			if ($flags & memcached::SERIALIZED) $ret[$rkey] = unserialize($ret[$rkey]);
		 } else {
	...
	*/
	function _set ($cmd, $key, $val, $exp) {
		...
		if (!is_scalar($val)) {
			...
			if ($_GET['dump']=='MEM') $this->_debugprint(sprintf("client: serializing data as it is not scalar\n"));
		}
		
		$len = strlen($val);
		
		if ($this->_have_zlib && $this->_compress_enable && $this->_compress_threshold && $len >= $this->_compress_threshold) {
			$c_val = gzcompress($val, 9);
			$c_len = strlen($c_val);
			
			if ($c_len < $len*(1 - memcached::COMPRESSION_SAVINGS)) {
				if ($_GET['dump']=='MEM') $this->_debugprint(sprintf("client: compressing data; was %d bytes is now %d bytes\n", $len, $c_len));
				$val = $c_val;
				$len = $c_len;
		...
		if ($_GET['dump']=='MEM') $this->_debugprint(sprintf("%s %s (%s)\n", $cmd, $key, $line));
	...
	*/
	function sock_to_host ($host) {
		if (isset($this->_cache_sock[$host])) return $this->_cache_sock[$host];
		$sock = null;
		$now = time();
		list ($ip, /* $port */) = explode (":", $host);
		if (isset($this->_host_dead[$host]) && $this->_host_dead[$host] > $now || isset($this->_host_dead[$ip]) && $this->_host_dead[$ip] > $now) {
			return null;
		}
		if (!$this->_connect_sock($sock, $host)) return $this->_dead_sock($host);
	...
	function _debugprint($str) {
		global $Conf;
		$bk = debug_backtrace();
		$d = '<b'.($bk[2]['file']?' title="'.$bk[2]['file'].' on line '.$bk[2]['line'].'"':'').'>'.$bk[2]['function'].'(</b>'.Debug::printArguments($bk[2]['args']).'<b>)</b><br><span style="color:#666">and</span> <b'.($bk[3]['file']?' title="'.$bk[3]['file'].' on line '.$bk[3]['line'].'"':'').'>'.$bk[3]['function'].'(</b>'.Debug::printArguments($bk[3]['args']).'<b>)</b>';
		$Conf->fill('MEMcache_DEBUG','<span style="font:12px monospace">'.$str.'</span><div style="border-bottom:2px solid #ccc;text-align:right;font:10px Arial;background:#dedede;color:#333333"><span style="color:#666">in</span> '.$d.'</div>');
	}

 ...
 class pChart
  {
   ...
   var $Errors         = NULL;
   var $ErrorFontName  = "./config/fonts/comic.ttf";
   var $ErrorFontSize  = 6;
...
// * Fixed in 1.27 *         { $X2 = $this->GArea_X2 - 1; $Y2 = $this->GArea_Y2 - ($this->GArea_X2 - $X1); }
   ...
   function drawScale($Data,$DataDescription,$ScaleMode,$R,$G,$B,$DrawTicks=TRUE,$Angle=0,$Decimals=1,$WithMargin=FALSE,$SkipLabels=1,$RightScale=FALSE) {
     ...
     if ( $this->VMin == NULL && $this->VMax == NULL) {
       ...
       if ( $this->VMin == 0 && $this->VMax == 0 ) { $this->VMin = 0; $this->VMax = 2; $Scale = 1; $Divisions = 2;}
       elseif ($MaxDivs > 1) {
         while(!$ScaleOk) {
   ...
   function drawXYScale($Data,$DataDescription,$YSerieName,$XSerieName,$R,$G,$B,$WithMargin=0,$Angle=0,$Decimals=1) {
     ...
     if ( $this->VXMin == NULL && $this->VXMax == NULL) {
       ...
       if ( $this->VXMin == 0 && $this->VXMax == 0 ) { $this->VXMin = 0; $this->VXMax = 2; $Scale = 1; $XDivisions = 2;}
       elseif ($MaxDivs > 1) {
         while(!$ScaleOk) {
...
/* Compute and draw the scale */
function drawGrid($LineWidth,$Mosaic=TRUE,$R=220,$G=220,$B=220,$Alpha=100, $StepX = 1, $StepY = 1) {
 /* Draw mosaic */
 if ( $Mosaic ) {
   ...
   $LastY = $YPos;
   for($i=0;$i<=$this->DivisionCount;$i+=$StepY) {
	 $LastY = $YPos;
	 $YPos  = $YPos - $this->DivisionHeight;
 ...
 /* Horizontal lines */
 if ($StepY) {
 $YPos = $this->GArea_Y2 - $this->DivisionHeight + $StepY;
 for($i=1;$i<=$this->DivisionCount;$i+=$StepY) {
   if ( $YPos > $this->GArea_Y1 && $YPos < $this->GArea_Y2 )
	$this->drawDottedLine($this->GArea_X1,ceil($YPos),$this->GArea_X2,$YPos,$LineWidth,$R,$G,$B);
	
   $YPos = $YPos - $this->DivisionHeight + $StepY - 1;
  }
 }
 if ($StepX) {
 /* Vertical lines */
 if ( $this->GAreaXOffset == 0 ) { $XPos = $this->GArea_X1 + ($StepX > 1 ? $StepX : $this->DivisionWidth) - 1 + $this->GAreaXOffset; $ColCount = $this->DataCount; }
 else
  { $XPos = $this->GArea_X1 + $this->GAreaXOffset; $ColCount = floor( ($this->GArea_X2 - $this->GArea_X1) / $this->DivisionWidth ); }


 //if ($StepX) $ColCount = $ColCount / ceil($StepX / 2);
 for($i=1;$i<=$ColCount;$i++) {
   ...
   $XPos = $XPos + $this->DivisionWidth + $StepX - 0.2;
  }
 }
}
     ...
     foreach ( $DataDescription["Values"] as $Key2 => $ColName ) {
       ...
       foreach ( $Data as $Key => $Values ) {
         ...
         if ( !is_numeric($Value) ) {
          ...
          {
           $PointsCount++;
           if ( $YLast <> $Empty ) { $aPoints[] = $XPos; $aPoints[] = $YPos; }
           else
   ...
   function drawFlatPieGraph($Data,$DataDescription,$XPos,$YPos,$Radius=100,$DrawLabels=PIE_NOLABEL,$SpliceDistance=0,$Decimals=0,$AllBlack=FALSE) {
     ...
     foreach($iValues as $Key => $Value) {
       ...
       if ( $AllBlack ) { $Rc = $this->ShadowRColor; $Gc = $this->ShadowGColor; $Bc = $this->ShadowBColor; }
       else
   ...
   function drawEllipse($Xc,$Yc,$Height,$Width,$R,$G,$B) { $this->drawCircle($Xc,$Yc,$Height,$R,$G,$B,$Width); }

   /* This function will draw an ellipse */
   function drawFilledEllipse($Xc,$Yc,$Height,$Width,$R,$G,$B) { $this->drawFilledCircle($Xc,$Yc,$Height,$R,$G,$B,$Width); }
   ...
   function drawFromPNG($FileName,$X,$Y,$Alpha=100) { $this->drawFromPicture(1,$FileName,$X,$Y,$Alpha); }

   /* Load a GIF file and draw it over the chart */
   function drawFromGIF($FileName,$X,$Y,$Alpha=100) { $this->drawFromPicture(2,$FileName,$X,$Y,$Alpha); }

   /* Load a JPEG file and draw it over the chart */
   function drawFromJPG($FileName,$X,$Y,$Alpha=100) { $this->drawFromPicture(3,$FileName,$X,$Y,$Alpha); }
